...
public class DocumentParserFactory {
    ...
    private static final Map<FileTypeEnum, Map<ParseTypeEnum, List<Class<? extends DocumentParser<String, ?>>>>> PARSER_MAP = new HashMap<>();

    // 缓存实例化后的解析器对象
    private static final ConcurrentHashMap<String, DocumentParser<String, ?>> CACHE = new ConcurrentHashMap<>();
...
//        registerParser(FileTypeEnum.PDF, ParseTypeEnum.TABLE, PyPDFPlumberTableParser.class);
     ...
     * 注册解析器到映射表中。
     */
    public static void registerParser(FileTypeEnum fileType, ParseTypeEnum parserType, Class<? extends DocumentParser<String, ?>> parserClass) {
    ...
    public static DocumentParser<String, ?> createParser(FileTypeEnum fileType, ParseTypeEnum parserType) {
     ...
     */
    @SuppressWarnings("unchecked")
    public static DocumentParser<String, ?> createParser(FileTypeEnum fileType, ParseTypeEnum parserType, Class<? extends AbstractTableParser<String>> customparserclass, boolean useCache) {
        if (customparserclass != null) {
            ...
            List<Class<? extends DocumentParser<String, ?>>> parserClasses = PARSER_MAP.getOrDefault(fileType, Maps.newHashMap())
     ...
     */
    @SuppressWarnings("unchecked")
    private static DocumentParser<String, ?> instantiateParser(Class<? extends DocumentParser<String, ?>> parserClass, boolean useCache, String... cacheKey) {
        try {
     ...
     * 根据文件路径和解析类型创建解析器。
     */
    public static DocumentParser<String, ?> createParserByFilePath(String filePath, ParseTypeEnum parserType) {
     ...
     */
    public static DocumentParser<String, ?> createParserByFilePath(String filePath, ParseTypeEnum parserType, Class<? extends AbstractTableParser<String>> customParserClass) {
        FileTypeEnum fileType = getFileTypeByExtension(filePath);
     ...
     * 获取所有注册的解析器类。
     */
    public static List<Class<? extends DocumentParser<String, ?>>> getParsers(FileTypeEnum fileType, ParseTypeEnum parserType) {
