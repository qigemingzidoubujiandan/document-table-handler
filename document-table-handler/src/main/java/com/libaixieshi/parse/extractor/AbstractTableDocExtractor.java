...
import com.libaixieshi.parse.domain.PDFTable;
...
@Slf4j
@Data
public abstract class AbstractTableDocExtractor<T extends ExtractedResult> implements DocumentExtractor<List<Table>, T> {

    protected final ExtractorConfig config;
    protected T parsedResult;

    protected AbstractTableDocExtractor(ExtractorConfig config) {
        this.config = config;
    ...
    public static void delEmptyRow(List<? extends Table> tables) {
        ...
        tables.forEach(table -> table.setData(
                table.getData().stream()
                        .filter(row -> row.stream().anyMatch(cell -> !StrUtil.isEmpty(cell.text())))
                        .collect(Collectors.toList())
        ));
    }
