 ...
 */
@Slf4j
public class ListDocExtractor extends AbstractTableDocExtractor<TableExtractedResult> {

    public ListDocExtractor(ExtractorConfig config) {
        super(config);
    }
    ...
    private int extractTableCell(Table table, List<Function<String, String>> thFunc,
        ...
        for (int i = 0; i < thFunc.size(); i++) {
            ...
            for (List<Cell> dataCells : rows) {
                Cell cell = CollectionUtil.get(dataCells, i);
